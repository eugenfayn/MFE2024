...
def show_overview():
    ...
    st.write()
    st.write("""
        **ParaView**  -  открытый графический кросс-платформенный пакет для интерактивной визуализации в исследовательских целях. ParaView полезен при визуализации расчётных сеток, визуализации полей, построении изоповерхностей в задачах механики сплошных сред и не только.
    """)
    st.subheader("Ключевые характеристики")
    st.markdown("""
- **Открытый исходный код и кроссплатформенность:**
Одной из основных характеристик ParaView является его открытый исходный код, что позволяет сообществу пользователей свободно модифицировать и расширять функциональность программы. ParaView поддерживает основные операционные системы, включая Windows, macOS и Linux.
- **Масштабируемость и распределённая обработка:** 
ParaView спроектирован для эффективной работы с очень большими объемами данных, которые часто возникают в результате комплексных научных расчетов и симуляций. Благодаря поддержке распределённой обработки данных, ParaView может использоваться для визуализации результатовна кластерах и суперкомпьютерах, распределяя задачи и ресурсы между множеством вычислительных узлов. Собственно, поэтому он и называется **Para**llel**View**
- **Гибкая система визуализации:** 
ParaView предоставляет пользователям широкий спектр инструментов визуализации, таких как объемный рендеринг, визуализация векторных и тензорных полей, изо-поверхностей и так далее.
- **Поддержка множества форматов данных:** 
ParaView может работать с разнообразными форматами данных, что облегчает его интеграцию в различные научные и инженерные рабочие процессы. Программа поддерживает многие популярные форматы VTK, COSMO, OpenFOAM, STL, XYZ и другие.
