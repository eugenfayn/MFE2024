...
def cnab(trange=None, inivel=None, inip=None, bcs_ini=[],
    ...
    v_n, p_n, bcs_n, bfv_n, mbc_c, mbc_n, fv_n, nfc_c, _, dfv_n, drm \
...
def _onestepheun(vc=None, pc=None, tc=None, tn=None,
    ...
    prjstkres = lau.app_prj_via_sadpnt(rhsv=stkres, amat=A, jmat=J,
                                       transposedprj=True)
    logging.debug(f'prjctd stokes res: {np.linalg.norm(prjstkres)}')
    extprjstkres = lau.app_prj_via_sadpnt(rhsv=extstkres, amat=A, jmat=J,
                                          transposedprj=True)
    logging.debug(f'extnd prjctd stokes res: {np.linalg.norm(extprjstkres)}')
    imexres = M@nvc + dt*A@nvc + dt*J.T@npc - dt*fv_n - M@nvc
    logging.debug(f'imex res: {np.linalg.norm(imexres)}')
    tv_n = tvp_n[:NV, :]
    tp_n = 1./dt*scalep*tvp_n[NV:, :]
    rimexres = M@tv_n + dt*A@tv_n + J.T@tvp_n[NV:, :] - dt*fv_n - M@nvc
    logging.debug(f'act imex res: {np.linalg.norm(rimexres)}')
    rhsdiff = np.linalg.norm(tfv - dt*fv_n-M@nvc)
    logging.debug(f'rhs diff: {rhsdiff}')
    import scipy.sparse as sps
    ...
    logging.debug(f'diff nvc-mvc: {np.linalg.norm(nvc-mvc)}')
    logging.debug(f'diff tnv-mvc: {np.linalg.norm(tv_n-mvc)}')
    mpcres = M@mvc + dt*A@mvc + J.T@mpc - tfv
    logging.debug(f'mpcres: {np.linalg.norm(mpcres)}')
    # import ipdb
    # ipdb.set_trace()
    # savevp(appndbcs(tv_new, tbcs), tp_new, time=(trange[1], 'heunpred'))
    logging.info(f'diff in update: {np.linalg.norm(tv_n - vc)}')
...
def semi_implicit_euler(iniv=None, jmat=None, mmat=None, amat=None,
                        rhsv=None, fp=None, rhsp=None,
                        trange=None, data_trange=None,
                        return_outputs=False, cpmat=None, cvmat=None,
                        return_v_only=False
                        ):
    ...
    def _rhsp(t, vvec):
        return np.zeros((NP, 1))

    _fpvt = _rhsp if rhsp is None else rhsp
    ...
    if return_outputs:
        if cvmat is None:
            rcv = RecordPOutput(data_trange=ie_dtpt_trng, cpmat=cpmat)
        if cpmat is None:
            rcv = RecordVOutput(data_trange=ie_dtpt_trng, cvmat=cvmat)
        else:
            rcv = RecordVPOutput(data_trange=ie_dtpt_trng,
                                 cvmat=cvmat, cpmat=cpmat)
    else:
        rcv = RecordSimulationDataPlain(data_trange=ie_dtpt_trng,
                                        only_v=return_v_only)
    ...
    try:
        for ct in track(trange[1:], description='semi-IE ongoing'):
            ...
            cvpn = d_impeul_increment(ct, cvpn[0])
            rcv.write_value(ct, cvpn)
    except RuntimeWarning:
        return -1
    return rcv.get_values()


class RecordSimulationDataPlain:
    def __init__(self, data_trange=None, cvmat=None, cpmat=None,
                 only_v=False):
        self.int_list = []
        self.dtr = data_trange
        self.cvmat = cvmat
        self.cpmat = cpmat
        self.only_v = only_v

    def what_to_write(self, cvp):
        if self.only_v:
            return cvp[0]
        else:
            return cvp

    def write_value(self, ct, cvp):
        try:
            if ct == self.dtr[0]:
                self.int_list.append(self.what_to_write(cvp))
                self.dtr.pop(0)
            else:
                pass
        except IndexError:
...
class RecordPOutput(RecordSimulationDataPlain):
    def what_to_write(self, cvp):
        return (self.cpmat @ cvp[1]).flatten()

class RecordVOutput(RecordSimulationDataPlain):
    def what_to_write(self, cvp):
        return (self.cvmat @ cvp[0]).flatten()

class RecordVPOutput(RecordSimulationDataPlain):
    def what_to_write(self, cvp):
        cvout = (self.cvmat @ cvp[0]).flatten()
        cpout = (self.cpmat @ cvp[1]).flatten()
        return (cvout, cpout)
